...
- Need to build a document Q&A system? Start with Chapter 6 (RAG from First Principles) or Chapter 10 (LlamaIndex).
- Need to query a SQL database with natural language? Go to Chapter 8 (RAG with Structured Data).
- Need to build a multi-agent workflow? Chapters 14–17 cover single-agent, multi-agent, autonomous, and tool-using architectures.
- Need to fine-tune a model on company-specific data? Chapter 13 (QLoRA) has a complete working example.
- Need to deploy an AI tool as a cloud service? Chapter 18 covers MCP servers on Supabase and no-code workflows with Dify.

Each notebook is self-contained — you can run it independently without working through the preceding chapters. The `llm_cascade` package handles API key management and provider fallback, so you only need to set one API key to get any notebook running.
